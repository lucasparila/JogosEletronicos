...
public class DAO {
    ...
    private static final String CATALOGO_GERAL = "catalogoGeral";
    private static final String CATALOGO_PERSONALIZADO = "catalogoJogos";

    @SuppressWarnings("unchecked")
    private static ArrayList<Jogo> getLista(ServletContext context, TipoCatalogo tipo) {
        String atributo = (tipo == TipoCatalogo.GERAL) ? CATALOGO_GERAL : CATALOGO_PERSONALIZADO;
    ...
    public static void inicializarCatalogo(ServletContext context) {
        ArrayList<Jogo> catalogo = getLista(context, TipoCatalogo.GERAL);
        ...
        catalogo.add(new Jogo("Elden Ring", "FromSoftware", "2022", "RPG", 
            "Elden Ring é um jogo eletrônico de RPG de ação que segue a história de um Tarnished, um personagem banido que busca reparar o Elden Ring para conquistar a glória e se tornar um Elden Lord após uma grande guerra entre deuses antigos. O jogo se passa nas Terras Intermédias, um mundo governado pela Rainha Marika, onde o jogador deve enfrentar desafios e explorar para alcançar seus objetivos. A narrativa é rica em lore e está profundamente entrelaçada com a história do mundo e seus personagens.", "Inglês", "PC", "18", "imagens/elden_ring.jpg"));
    ...
    public static ArrayList<Jogo> getJogos(ServletContext context, TipoCatalogo tipo) {
        return getLista(context, tipo);
    }

    public static Jogo buscarJogoNome(String nome, ServletContext context, TipoCatalogo tipo) {
        ArrayList<Jogo> jogos = getLista(context, tipo);
        for (Jogo j : jogos) {
            if (j.getTitulo().equalsIgnoreCase(nome)) {
                return j;
            }
        }
        return null;
    ...
    public static void adicionarJogo(Jogo jogo, ServletContext context, TipoCatalogo tipo) {
        ArrayList<Jogo> jogos = getLista(context, tipo);
        jogos.add(jogo);
    }

    public static void removerJogo(Jogo jogo, ServletContext context, TipoCatalogo tipo) {
        ArrayList<Jogo> jogos = getLista(context, tipo);
        jogos.remove(jogo);
    }

    public static void atualizarJogo(Jogo jogoAtualizado, ServletContext context, TipoCatalogo tipo) {
        Jogo jogo = buscarJogoNome(jogoAtualizado.getTitulo(), context, tipo);
        if (jogo != null) {
            ...
            jogo.setImagemJogo(jogoAtualizado.getImagemJogo());
        }
    }
}
